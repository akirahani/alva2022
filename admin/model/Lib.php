<?php
	class Lib{
		...
		public function changeSlug($title, $separator = '-')
	    {
	        ...
	        {
	            $arr=explode("|",$codau);
	            $title = str_replace($arr, $khongdau, $title);
	        }
		...
		public function HoTen($str)
	    {
	        $y = explode(" ", trim($str));
	        $result = count($y) - 1;
	        $f = strtoupper($y['0']);
	        $l = strtoupper($y[$result]);
	        if(count($y)==1)
	        {
	            $str=$l[0];
	        }
	    ...
	    public function GiaiMa($str, $type)
	    {
			$method = 'aes-256-cbc';
